(function($) {
			...
			.on(
					...
					function(e) {
						...
						$this.parent().siblings().removeClass(
								'sidenav__menu--is-open');
						$this.parent().siblings().find('li').removeClass(
								'sidenav__menu--is-open');
						...
						if ($this.next().hasClass('show')) {
							...
							$this.parent().parent().find(
									'li .sidenav__menu-dropdown').removeClass(
									'show').slideUp(350);
						...
						function() {
							function loadDaBars() {
								$('.progress-bars__base')
										.each(
												function(index) {
													var $this = $(this), bar = $this
															.find('.progress-bars__bar'), barWidth = bar
															.attr('aria-valuenow');
													setTimeout(function() {
														bar.css({
															"width" : barWidth
	...
	var submitContact = $('#submit-message'), message = $('#msg');
									...
									success : function(data) {
										...
										if (data.info !== 'error') {
											$this
													...
													.filter(':visible').val('');
											message.hide().removeClass(
													'success').removeClass(
													'error')
													.addClass('success').html(
															data.msg).fadeIn(
															'slow').delay(5000)
													.fadeOut('slow');
										} else {
