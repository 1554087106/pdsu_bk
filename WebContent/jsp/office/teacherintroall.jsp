...
<title>平院百科| 老师列表</title>
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/topcontroller/office/css/bootstrap.min.css" />
<link rel="stylesheet" type="text/css"
	...
	<section style="background-color: #f7f7f7; padding: 0px 0px 30px 0px;">
		<div class="container">
			<div class="page-header">
				<div class="row">
					<div class="col-lg-12">
						<h1 class="pull-left">${c_name}的可爱老师们</h1>
						<form class="navbar-form navbar-right"
							action="${pageContext.request.contextPath}/findTeacherByName.action"
							method="post">
							<input type="text" name="t_name" class="form-control col-lg-8"
								placeholder="请输入老师的姓名">
						</form>
					</div>
				</div>
			</div>
				...
				<c:if test="${pageInfo.list!=null}">
					<c:forEach items="${pageInfo.list}" var="teacher">
						<div class="col-sm-4 animated" data-animation="bounceIn"
							data-delay="0">
							<div class="walkhi">
								<h3>姓名：${teacher.t_name}</h3>
								职称：${teacher.t_pro} <br /> <i class="fa fa-mobile-phone">:</i>${teacher.t_tel}
								<br> <i class="fa fa-envelope-o">:</i>${teacher.t_email} <br>
								所属办公室编号：${teacher.office.o_num} <br />
								所属办公室名称：${teacher.office.o_name} <br />
								所属院系：${teacher.college.c_name} <br />
								<p>办公室位置:中心会议室右侧科技楼二楼${teacher.office.o_name}</p>
								<c:if test="${teacher.flag==0}">
									<i class="fa fa-user-plus"></i>
									<a href="#">添加至我的导师</a>
								</c:if>
								<c:if test="${teacher.flag==1}">
									<i class="glyphicon glyphicon-heart-empty" style="color: red"></i>
									<a href="#" style="color: red">已添加</a>
								</c:if>
